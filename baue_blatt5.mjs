...
const band = (paintedId, leben, kraft) => {
  const m = MASS[paintedId]; const id = "sb" + paintedId;
  ...
  return `<svg viewBox="0 0 ${m.W} ${m.H}" preserveAspectRatio="xMidYMax meet" data-gg="sockelband"
    ...
    <defs>
      <linearGradient id="${u("rot")}" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#ff8a80"/><stop offset=".35" stop-color="#e5322c"/><stop offset="1" stop-color="#7a0e0e"/></linearGradient>
      <linearGradient id="${u("blau")}" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#8fb4ff"/><stop offset=".35" stop-color="#2d63e6"/><stop offset="1" stop-color="#0d2a7a"/></linearGradient>
...
<div class="marke">Entwurf 5 · das echte Sockelband, und nur noch ein Hintergrund
  <small>Das Band ist jetzt nicht mehr nachgebaut, sondern dieselbe Rechnung wie im Hofstaat — aus sockelband.json, in Bildpixeln, mit den gemessenen Ellipsen jeder Figur. Der helle violette Rahmen ist fort: die Bühne füllt das Blatt ganz aus.</small></div>
<div class="reihen">
...
fs.writeFileSync("/mnt/user-data/outputs/entwurf-figurenblatt-5.html", html);
...
await p.goto("file://" + path.resolve("/mnt/user-data/outputs/entwurf-figurenblatt-5.html"), { waitUntil: "load" });
await p.waitForTimeout(600);
await p.screenshot({ path: "/mnt/user-data/outputs/entwurf-figurenblatt-5.png", fullPage: true });
