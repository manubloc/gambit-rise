...
await klick("^Spielen$"); await page.waitForTimeout(1500);
await klick("Kampagne"); await page.waitForTimeout(2600);
await page.screenshot({ path: "/tmp/karte_uebersicht.png" });
await klick("Weiter zur Karte"); await page.waitForTimeout(2600);
await page.screenshot({ path: "/tmp/karte_uebersicht.png" });
/* die erste Station antippen */
const traf = await page.evaluate(() => {
  /* die Stationen sind anklickbare Punkte auf der Karte - wir nehmen den,
     der als naechster dran ist (der einzige helle). */
  /* erst ins Kapitel: die Marke "AKTUELL" unten auf der Weltkarte */
  const a = [...document.querySelectorAll("*")].find((e) => e.children.length === 0 && e.textContent.trim() === "AKTUELL");
  const ziel = a ? a.parentElement : null;
  if (ziel) { ziel.dispatchEvent(new MouseEvent("click", { bubbles: true })); return "Kapitel geoeffnet"; }
  const b = [...document.querySelectorAll("button")].map((x) => x.textContent.trim()).slice(0, 25);
  return "keine Marke; Knoepfe: " + JSON.stringify(b);
});
console.log(traf);
await page.waitForTimeout(2600);
await page.screenshot({ path: "/tmp/karte_kapitel.png" });
await klick("zur Karte"); await page.waitForTimeout(2600);
await page.screenshot({ path: "/tmp/karte_kapitel.png" });
/* jetzt eine Station antippen: der helle, spielbare Punkt */
const t2 = await page.evaluate(() => {
  const k = [...document.querySelectorAll("button")].filter((e) => { const r = e.getBoundingClientRect();
    return r.width > 20 && r.width < 80 && r.height > 20 && r.height < 80 && r.top > 70 && r.bottom < innerHeight - 80; });
  if (!k.length) return "keine Station";
  k[k.length - 1].dispatchEvent(new MouseEvent("click", { bubbles: true }));
  return "Station " + k.length;
});
console.log(t2);
await page.waitForTimeout(1800);
await page.screenshot({ path: "/tmp/karte_station.png" });
/* und einmal OHNE Fenster - fuer den ehrlichen Vergleich */
await page.evaluate(() => { const x = [...document.querySelectorAll("button")].find((b) => b.getAttribute("aria-label") === "Close" || b.textContent.trim() === "\u2715"); if (x) x.click(); });
await page.waitForTimeout(1200);
await page.screenshot({ path: "/tmp/karte_leer.png" });
