...
const band = (id, leben, kraft) => { const m = MASS[id]; if (!m) return "";
...
for (const f of dateien) {
  ...
  const kopf = null;
  zellen.push({ id, m, k, st, mitte: mitte(m), aus: ausgleich(m, id), datei: f,
...
const zelle = (z) => z.fehlt
  ...
  : `<div class="z">
    ...
    <div class="zahlen">rx ${z.m.rx.toFixed(0)} · ×${z.k.toFixed(2)} · ↕${z.st.toFixed(2)}<br>H ${z.hoehe} · ↔${z.mitte.toFixed(1)}%</div>
  </div>`;
...
<h1>Die Matrize — alle ${zellen.length} Gemälde mit Band und Skalierung</h1>
<p class="u">Gezeichnet mit genau derselben Rechnung wie das Spiel: Tellerbreite auf ${ZIEL_RX}, senkrechte Streckung auf die Höhe des Bauern (${ZIEL_HOEHE}), Ausrichtung auf den Teller, alle auf die gemeinsame Bodenlinie ${BODEN_LINIE} — die blaue Linie. Unter jeder Figur: gemessene Tellerbreite rx, Tellerfaktor ×, senkrechte Streckung ↕, die gezeigte Höhe H in Bildpixeln und die Seitwärtskorrektur ↔. Wo H stark von den anderen abweicht oder ↕ an 0,85 bzw. 1,20 klebt, ist die Kappung erreicht — dort lohnt das Nachjustieren.</p>
...
fs.writeFileSync("/mnt/user-data/outputs/matrize-figuren.html", html);
...
await p.goto("file://" + path.resolve("/mnt/user-data/outputs/matrize-figuren.html"), { waitUntil: "load" });
await p.waitForTimeout(900);
await p.screenshot({ path: "/mnt/user-data/outputs/matrize-figuren.png", fullPage: true });
await b.close();
