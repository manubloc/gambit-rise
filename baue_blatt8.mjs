...
const FAM = { schritt:["#123c2a","#0a2418","#6fe0a8","#c8f5dd"], sprung:["#12314e","#0a1d30","#6fc2f0","#cfeaff"],
  ...
  /* v-Entwurf 7: die AEUSSERE Kontur traegt jetzt satt die Bandfarbe, das
     Zeichen darin eine Spur heller - so hebt es sich von den Faehigkeiten ab,
     ohne die Sprache zu verlassen. */
  bandRot:["#571419","#2e080b","#ff9f96","#ffe6e2"],
  bandBlau:["#16295a","#0a1633","#a3c1ff","#e4eeff"] };
let nr = 0;
...
const zeichen = (fam, inner, size = 26) => { const [g,t,r,st] = FAM[fam]; const id = "v"+(++nr);
  ...
  <rect x="1" y="1" width="22" height="22" rx="6.2" fill="none" stroke="${r}" stroke-width="${fam.startsWith("band") ? 1.5 : 1}" opacity="${fam.startsWith("band") ? 1 : .85}"/>
...
/* GEMESSEN, warum die Dame schief sass: ihr Teller steht 39,5 px rechts der
   Bildmitte (cx 327,5 bei W 576). Das Spiel gleicht das mit
   tellerMitteProzent aus - im Entwurf fehlte es. Jetzt liegt dieselbe
   Rechnung drin: Tellerbreite auf 136, Hoehe auf 561, Ausrichtung auf den
   Teller, gemeinsame Bodenlinie 555. */
const ZIEL_RX = 136, ZIEL_HOEHE = 561, BODEN_LINIE = 555;
const gSkal = (m, id) => Math.max(0.55, Math.min(1.35, ZIEL_RX / m.rx));
const gStreck = (m, id) => { if (m.oben == null) return 1;
  return Math.max(0.85, Math.min(1.2, ZIEL_HOEHE / ((m.boden - m.oben) * gSkal(m, id)))); };
const gMitte = (m) => -((m.cx - m.W / 2) / m.W) * 100;
const gAus = (m, id) => { const k = gSkal(m, id) * gStreck(m, id);
  return Math.max(-8, Math.min(8, (((m.H - m.boden) * k - (m.H - BODEN_LINIE)) / m.H) * 100)); };
const figurStil = (id) => { const m = MASS[id];
  return `transform-origin:50% 100%;transform:translate(${gMitte(m).toFixed(2)}%, ${gAus(m, id).toFixed(2)}%) scale(${gSkal(m, id).toFixed(3)}, ${(gSkal(m, id) * gStreck(m, id)).toFixed(3)})`; };
...
/* ── v-Entwurf 7 (Besitzer): "vielleicht auch nicht die Kante runterziehen,
   sondern einfach deinen gestalteten Sockel komplett runterziehen, dass sich
   hier nicht so eine Kante ergibt."

   GEMESSEN, und es ist kein Einzelfall: das Band ist 17 % der Tellerbreite
   hoch, der GEMALTE Farbring der Teller aber nur 8 bis 24 px. Bei allen 69
   Figuren steht das Band deshalb ueber den gemalten Ring hinaus - beim
   Springer um 29, beim Haendler um 61, bei der Schatzkammer um 88 Bildpixel.
   Genau dieser Ueberstand ist die Kante.

   Statt das Band duenner zu machen (dann liest es sich auf der Kachel als
   Strich) haengt es jetzt UNTER der Bodenellipse: seine Oberkante liegt
   genau auf der gemalten Unterkante des Tellers, es gibt also nichts mehr,
   was ueberstehen koennte. Die Figur samt Band rutscht dafuer um dieselbe
   Hoehe nach oben - und weil der Sockel jetzt eigenen Platz hat, laesst sich
   die Groesse einzelner Figuren darin justieren. */
...
const BAND_ART = {
  ...
  /* Besitzer: "Schach ohne Werte - Gegner kannst du gleich machen wie das
     noch ohne Werte" - es braucht das dritte Grau nicht. */
};
const band = (paintedId, leben, kraft, art = "werte") => {
  ...
  if (art === "eigen") { leben = 0.5; kraft = 0.5; }
  const H = Math.round(m.rx * 2 * 0.17), rand = Math.max(2, H * 0.13);
  /* Besitzer, praeziser: "das Band soll schon immer den Sockel ueberdecken,
     mein Vorschlag war nur, das Band darf hoeher sein als der Sockel." Also
     wieder von der Bodenkante AUFWAERTS - es deckt den gemalten Ring zu. */
  const h = H;
...
const abz = (stufe, id) => `<svg width="40" height="40" viewBox="0 0 64 64" style="display:block;flex:0 0 auto"><defs><radialGradient id="ab${id}" cx="50%" cy="30%" r="76%"><stop offset="0%" stop-color="#f3d68d"/><stop offset="60%" stop-color="#c79a3e"/><stop offset="100%" stop-color="#7e5a1c"/></radialGradient></defs><circle cx="32" cy="32" r="26" fill="url(#ab${id})" stroke="#f6e2a8" stroke-width="2"/><circle cx="32" cy="32" r="20" fill="#2a1d0a" opacity=".5"/><text x="32" y="40" text-anchor="middle" font-size="22" font-weight="800" font-family="Georgia,serif" fill="#ffd27a">${stufe}</text></svg>`;
/* GEMESSEN: alle 26 Figuren haben zehn Stufen - nur der Grand Gambit hat
   SECHZIG. Der Besitzer moechte ihn auf zwanzig kuerzen und in zwei Reihen
   zu zehn zeigen. Hier ist beides gezeichnet. */
const pips = (n, gesamt = 10) => { const reihen = [];
  for (let r = 0; r * 10 < gesamt; r++) reihen.push(Array.from({ length: Math.min(10, gesamt - r * 10) },
    (_, i) => `<i class="${r * 10 + i < n ? "v" : ""}"></i>`).join(""));
  return `<div class="pips">${reihen.map((z) => `<div>${z}</div>`).join("")}</div>`; };

const blatt = ({ name, haus, satz, stufe, atk, hp, gelernt, gesamt, kul, fig, ton, id, leben, kraft, stufen }) => `
<div class="schirm">
  ...
  <div class="blatt"><div class="rumpf">
    <div class="buehne">
      ...
      <div class="kopfzeile"><div class="stufenzeile">${pips(stufe, stufen || 10)}${abz(stufe, id)}</div></div>
      <div class="zwei">
        <div class="links">
          <div class="figur"><img src="FIG_${id}" alt="" style="${figurStil(id === "k" ? "knight" : "queen")};filter:drop-shadow(0 0 12px ${ton}88) drop-shadow(0 3px 5px rgba(0,0,0,.55))">
      ...
      <div class="werte">
        <div class="wert">${zeichen("bandRot", KLINGE_STRICH)}
          <div class="paar"><div class="oben"><span class="z" style="color:#ffb3aa">${atk}</span><b>+1</b></div>
            <div class="unten"><span class="w" style="color:#e08a84">Angriff</span><span>nächste</span></div></div></div>
        <div class="wert">${zeichen("bandBlau", HERZ_STRICH)}
          <div class="paar"><div class="oben"><span class="z" style="color:#b6cdff">${hp}</span><b>+1</b></div>
            <div class="unten"><span class="w" style="color:#8ba6e0">Leben</span><span>nächste</span></div></div></div>
      </div>
      <div class="knopf"><span>Verbessern</span><span class="preis"><img src="${SKILL}" alt="">2</span></div>
    </div>
  </div></div></div>
...
.pips{display:flex;flex-direction:column;gap:3px}
.pips div{display:flex;gap:3.5px}
.pips i{width:9px;height:6px;border-radius:3px;background:rgba(255,255,255,.12);display:block}
...
/* Zahl eine Spur kleiner, und die beiden Woerter stehen auf EINER Zeile -
   "Angriff" links, "nächste" rechts, gleiche Grundlinie. */
.paar{flex:1;display:flex;flex-direction:column;gap:4px}
.paar .oben,.paar .unten{display:flex;align-items:baseline;justify-content:space-between;gap:8px}
.wert .z{font:800 15px/1 Georgia,serif}
.wert .w{font:600 8.5px/1 Georgia,serif;letter-spacing:.11em;text-transform:uppercase}
.paar .oben b{font:800 11.5px/1 Georgia,serif;color:var(--rissHell);text-shadow:0 0 7px rgba(167,139,250,.75)}
.paar .unten span:last-child{font:600 7.5px/1 Georgia,serif;letter-spacing:.09em;text-transform:uppercase;color:#9a8fc0}
.preis{display:inline-flex;align-items:center;justify-content:center;gap:5px;padding:4px 10px 4px 7px;border-radius:8px;
  background:rgba(8,5,16,.55);border:1px solid rgba(167,139,250,.45);font:800 13px/1 Georgia,serif;color:#f2e7c8;text-shadow:none}
.preis img{width:17px;height:17px;display:block;filter:drop-shadow(0 0 4px rgba(139,92,246,.6))}
...
<div class="marke">Entwurf 8 · Band wieder über dem Sockel, Dame gerade, Stufenanzeige größer
  <small>Die Dame saß schief, weil im Entwurf die Ausrichtung auf den Teller fehlte — ihr Teller steht 39,5 px rechts der Bildmitte. Jetzt rechnet der Entwurf genau wie das Spiel.</small></div>
<div style="display:flex;align-items:center;gap:14px;margin-bottom:18px;padding:10px 14px;border-radius:12px;background:rgba(16,11,30,.7);border:1px solid #2f2450;width:fit-content">
  <span style="font:600 10px/1.3 Georgia,serif;letter-spacing:.1em;text-transform:uppercase;color:#c9b26a">Nur der Grand Gambit — zwanzig Stufen, zwei Reihen</span>
  ${pips(13, 20)}${abz(13, "gg")}
  <span style="font:400 10.5px/1.4 Georgia,serif;color:#837e6f;max-width:280px">Gemessen: alle 26 Figuren haben zehn Stufen, der Gambit hat <b style="color:#e9cf8a">sechzig</b>.</span>
</div>
<div class="reihen">
${blatt({ stufen: 10, id: "k", name: "Springer", haus: "Freie Figur", satz: "Reitet Winkel, die keine Mauer je bedacht hat.", stufe: 5, atk: 4, hp: 7, gelernt: 2, gesamt: 5, ton: "#d64b3c", leben: .72, kraft: .55 })}
${blatt({ stufen: 10, id: "d", name: "Dame", haus: "Bund · Krone", satz: "Sie geht, wohin sie will, und niemand fragt warum.", stufe: 10, atk: 7, hp: 12, gelernt: 7, gesamt: 10, ton: "#c2416b", leben: .95, kraft: .8 })}
</div>
...
<div style="display:flex;gap:18px;flex-wrap:wrap">
  ${["werte","leer","eigen"].map((art, i) => `<div style="width:190px;display:flex;flex-direction:column;gap:6px;align-items:center">
    ...
    <div style="font:600 10px/1.35 Georgia,serif;letter-spacing:.06em;color:#cbbf9a;text-align:center">
      ${["Mit Leben und Kraft<br><span style='color:#837e6f'>rot links, blau rechts</span>",
         "Ohne Werte · auch Gegner<br><span style='color:#837e6f'>schwarz, Fassung bleibt</span>",
         "Schach ohne Werte · eigene<br><span style='color:#837e6f'>hellgrau — Gegner wie Mitte</span>"][i]}</div>
  </div>`).join("")}
</div>`;
...
fs.writeFileSync("/mnt/user-data/outputs/entwurf-figurenblatt-8.html", html);
...
await p.goto("file://" + path.resolve("/mnt/user-data/outputs/entwurf-figurenblatt-8.html"), { waitUntil: "load" });
await p.waitForTimeout(600);
await p.screenshot({ path: "/mnt/user-data/outputs/entwurf-figurenblatt-8.png", fullPage: true });
