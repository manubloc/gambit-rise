...
/* ALLE Gemaelde, nicht nur die Ungeheuer - Figuren zuerst, dann die Bosse */
const ids = Object.keys(M)
  .filter((id) => M[id] && M[id].rx && fs.existsSync("src/app/ui/assets/painted/painted-" + id + ".webp"))
  .sort((a, b) => (/^boss/.test(a) ? 1 : 0) - (/^boss/.test(b) ? 1 : 0) || a.localeCompare(b));
...
<h1>Alle 69 Gemälde, wie sie auf dem Schachfeld stehen</h1>
...
fs.writeFileSync("/mnt/user-data/outputs/alle-auf-feld.html", html);
const { chromium } = await import("playwright-core");
const b = await chromium.launch({ executablePath: process.env.PW_CHROMIUM || undefined });
const p = await (await b.newContext({ viewport: { width: 1180, height: 1400 }, deviceScaleFactor: 2.5 })).newPage();
await p.goto("file://" + path.resolve("/mnt/user-data/outputs/alle-auf-feld.html"), { waitUntil: "load" });
await p.waitForTimeout(700);
await p.screenshot({ path: "/mnt/user-data/outputs/alle-auf-feld.png", fullPage: true });
await b.close();
console.log("Gemaelde:", ids.length, "| gekappt:", ids.filter((id) => Math.abs(st(M[id]) - 1.10) < 1e-9).length);
