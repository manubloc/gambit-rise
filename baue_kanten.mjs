...
const ids = Object.keys(NEU).sort();
const zelle = (id) => { const m = MASS[id], n = NEU[id];
  ...
  const zoom = 3.2, yv = (m.boden - m.H * 0.055) / m.H * 100;
  return `<div class="z">
...
.gitter{display:grid;grid-template-columns:repeat(6,1fr);gap:13px}
...
<h1>Prüfblatt — auf den Teller gezoomt (3,2-fach)</h1>
...
fs.writeFileSync("/mnt/user-data/outputs/pruefblatt-tellerkante.html", html);
...
await p.goto("file://" + path.resolve("/mnt/user-data/outputs/pruefblatt-tellerkante.html"), { waitUntil: "load" });
await p.waitForTimeout(900);
await p.screenshot({ path: "/mnt/user-data/outputs/pruefblatt-tellerkante.png", fullPage: true });
await b.close();
